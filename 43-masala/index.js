...
//======================================
// link => https://www.codewars.com/kata/52dda52d4a88b5708f000024/train/javascript
// 6 kyu

function ordinal(number, brief) {
  let arr = ["th", "st", "nd", "rd", "th", "th", "th", "th", "th"];
  let num = [11, 12, 13];
  let num2 = [2, 3];
  if (
    num.includes(+`${number}`.slice(`${number}`.length - 2, `${number}`.length))
  ) {
    return "th";
  } else {
    return brief &&
      num2.includes(
        +`${number}`.slice(`${number}`.length - 1, `${number}`.length)
      )
      ? arr[
          +`${number}`.slice(`${number}`.length - 1, `${number}`.length)
        ].slice(1, 2)
      : arr[+`${number}`.slice(`${number}`.length - 1, `${number}`.length)];
  }
}
console.log(ordinal(1, true));
